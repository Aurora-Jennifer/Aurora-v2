...
The feature selection pipeline now supports GPU acceleration via LightGBM, which can provide **10-50x speedup** compared to CPU processing, especially for large datasets with many features.
...
**Need help?** Check logs for GPU initialization messages, or run with a single symbol to isolate issues.
